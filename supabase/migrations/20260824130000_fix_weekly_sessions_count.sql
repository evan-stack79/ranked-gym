-- Assiduité : compter toutes les séances (notes), pas seulement les séries avec poids > 0.
-- Semaine ISO Postgres = lundi → dimanche (date_trunc('week')).
...
begin
  ...
  -- Lundi 00:00 → dimanche (semaine calendaire Postgres / ISO).
  ...
  sessions_last_7 as (
    ...
    from notes
    where date_key::date >= (current_date - interval '6 days')
  ),
  -- Toutes les notes validées dans [lundi, dimanche], pas seulement les sets pondérés.
  sessions_this_week as (
    select count(*)::integer as cnt
    from notes
    where date_key::date >= v_week_start
      and date_key::date <= v_week_end
  ),
  scores as (
    select
