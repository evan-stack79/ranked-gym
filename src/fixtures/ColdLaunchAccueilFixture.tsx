...
export function ColdLaunchAccueilFixture() {
  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col mesh-bg font-sans">
      ...
      <main
        className={`relative z-10 mx-auto w-full max-w-lg flex-1 px-5 py-8 ${coldEntering ? 'home-cold-enter home-cold-enter--active' : ''}`}
        style={{
          paddingBottom:
      ...
      <footer data-bottom-nav-host className="fixed bottom-0 left-0 right-0 z-40">
        <div className="mx-auto flex h-[calc(var(--app-bottom-nav)+env(safe-area-inset-bottom,0px))] w-full max-w-lg items-center justify-around border-t border-white/10 bg-[#111114]/92 px-4 pb-[env(safe-area-inset-bottom,0px)] backdrop-blur">
